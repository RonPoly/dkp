...
-- DKPPrice is price if the item is a valid raid drop, nil elseif.
  ...
  if zoneEng == "Icecrown Citadel" then
    zonaDrop='IC'
  elseif zoneEng == "Blackwing Decent" then
    zonaDrop='BWD'
  elseif zoneEng == "The Bastion of Twilight" then
    zonaDrop='TBOT'
  elseif zoneEng == "Throne of the Four Winds" then
    zonaDrop='TOTFW'
...
function QDKP2_IsMasterLooter()
	...
	if masterlooterRaidID and masterlooterRaidID==GetNumRaidMembers() then return true; end
end
...
function QDKP2_GiveLootToPlayer(item, name, simul)
  ...
  local lootnum=GetNumLootItems()
	...
	local nameSlot
	for i=1,50 do
		if GetMasterLootCandidate(i)==name then nameSlot=i; break; end
	end
